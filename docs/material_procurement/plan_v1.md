...
> **⚠️ 후속 문서**: STEEL 치수 관리 모델은 **[plan_v2.md](./plan_v2.md)** 에서 대체됨.
> v1의 STEEL 치수(W/L/H)가 Material 마스터에 고정되는 모델 → v2에서 **PO Item / PR / SteelTag 레벨로 분리**.
> 동일 강종(AL6061 등)을 다양한 치수로 발주하는 HMLV 패턴을 지원.
> v1의 카테고리 5종 + TOOL/CONSUMABLE 확장, 이중 단위(KG/EA), 태그 워크플로우는 **그대로 유효**.
